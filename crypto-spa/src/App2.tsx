...
/**
 * Crypto Prices SPA — Single‑file React app (with fixes)
 * -------------------------------------------------------------
 * Fixes in this version
 * 1) Dark/Light toggle works with Tailwind v4 when you add this ONE line to src/index.css:
 *      @custom-variant dark (&:where(.dark, .dark *));
 *    (This makes `dark:` classes respond to the `.dark` class on <html>.)
 * 2) The list now expands to full width when no coin is selected. When a coin is selected,
 *    the main content gets right padding equal to the drawer width so nothing is hidden.
 * 3) The detail panel now SLIDES IN smoothly from the right with a CSS transform transition.
 * 4) Chart bottom margin increased so the "Data source…" text never overlaps x‑axis labels.
 */
...
function useTheme() {
  ...
  useEffect(() => {
    ...
    // Helpful for built-in form controls, etc.
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      ...
      <header className="sticky top-0 z-10 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:supports-[backdrop-filter]:bg-zinc-900/60 bg-white/90 dark:bg-zinc-900/80 border-b border-zinc-200 dark:border-zinc-800">
        <div
          className={classNames(
            "mx-auto max-w-6xl px-4 sm:px-6 py-3 flex items-center gap-3 transition-[padding] duration-300",
            selected && "lg:pr-[580px]"
          )}
        >
      ...
      <section
        className={classNames(
          "mx-auto max-w-6xl px-4 sm:px-6 mt-4 transition-[padding] duration-300",
          selected && "lg:pr-[580px]"
        )}
      >
      ...
      <main
        className={classNames(
          "mx-auto max-w-6xl px-4 sm:px-6 mt-4 pb-24 transition-[padding] duration-300",
          selected && "lg:pr-[580px]"
        )}
      >
        {error && (
